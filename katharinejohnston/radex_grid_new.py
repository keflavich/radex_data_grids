...
tmax = 500.0  # maximum kinetic temperature (K)
nmin = 1e3   # minimum H2 density (cm^-3)
nmax = 1e8   # maximum H2 density (cm^-3)
cmin = 1e13  # minimum molecular column density
cmax = 1e15  # maximum molecular column density

ntemp = 31     # number of temperature points
ndens = 31     # number of density points
ncol  = 3     # number of column points
...
dv    = 10.          # line width (km/s)

mole = 'ph2co-h2'  # molecular data name

# Naming suffix to append to line name defined in "acts" below
suffix = "_test5_h2co_p"
# acts = list of lines to ratio.  Filenames must include ".dat" in order for suffix to be applied
acts = ([218.222,218.760,'303-202_321-220.dat'],)
flow = 217.5     # lowest frequency transition to include in output file
fupp = 220.0   # highest frequency transition to include in output file
...
#
def write_input(infile,tkin,nh2,cdmol):
    """
...
from read_radex_new import read_radex
    ...
    for temp in temperatures:
        for dens in densities:
            for col in columns:
                ...
                output =  read_radex(outfile,lowfreq,uppfreq,debug=True)
                print output
                temp,dens,col,tlow,tupp,taulow,tauupp,trotlow,trotupp,fluxlow,fluxupp = output
